...
const description = ref('结合机理模型、数据模型、数字孪生技术、多变量预测控制技术，解决钢铁企业生产工艺、设备控制等一系列工业生产痛点，实现工艺流、生产信息流、设备流"三流合一"，稳定提升成品质量，降低生产线设备故障风险、人力成本和能源消耗。')
...
const handleWheel = (e: WheelEvent) => {
  ...
  // 向下滚动
  if (e.deltaY > 0 && currentSection.value < 29) {
    currentSection.value++
    // currentSection.value = 29
  }
  // 向上滚动
...
const handleKeyDown = (e: KeyboardEvent) => {
  ...
  // 向下箭头或Page Down
  if ((e.key === 'ArrowDown' || e.key === 'PageDown') && currentSection.value < 30) {
    currentSection.value++
  }
  // 向上箭头或Page Up
